...
begin
//andro:=YearOf(DateTimePicker1.Date +'-'+DateTimePicker2.Date);
DataModule2.Adotable9.Append;
DataModule2.AdoTable9.insert;
DataModule2.AdoTable9.FieldByName('Trimestre').Value:=UpperCase(Edit1.Text);
...
Showmessage('Trimestre supprimer avec succé !');
end;
...
begin
DataModule2.ADOTable9.Delete;
Showmessage('Triméstre supprimé avec succé !');
DataModule2.ADOQuery7.Refresh;
end;
...
begin
  (*AdoQuery7.Close;
  AdoQuery7.ClearFields;
  AdoQuery7.SQL.Add('SELECT Trimestre.Trimestre, Sum(DateDiff("m",[Debut],[Fin]))&" mois" AS [Durée ]FROM Trimestre GROUP BY Trimestre.Trimestre;');
  AdoQuery7.Open;*)
